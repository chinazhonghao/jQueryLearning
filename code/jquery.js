...
(function (window, undefined) {
	...
	var jQuery = (function () {
		...
		var jQuery = function (selector, context) {
			...
			// (?:x)表示匹配x但是不记住匹配项，非捕获括号
			// [^xyz]表示反向字符集，匹配任何没有包含在方括号中的字符
			// * => {0,} 匹配0次或多次，相当于[^#<]可以不存在，但是不能以#或者<作为开始
			// \w匹配一个单字字符[A-Za-z0-9_]；\W匹配一个非单字字符[^A-Za-z0-9_]
			// <xxxx>前面不能跟随#或<符号，后面不能跟随>符号，但是前后都可以跟随其他字符
			// 提取一个HTML tag或者#后面跟随的字符
			quickExpr = /^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/,

			// Check if a string has a non-whitespace character in it
			// \S 匹配一个非空白字符（空格、制表符、换页符和换行符）
			rnotwhite = /\S/,
	...
	// 2. 或修改已有选项，
	// 3. 或将当前请求重定向到另一个数据类型
	// structure 指向前置过滤器集prefilters, 或请求发送器工厂函数集transports, 其值取决于调用该函数时传入的值
	function inspectPrefiltersOrTransports(structure, options, originalOptions, jqXHR,
		...
		// 去除数据类型对应的前置过滤器数组，或请求发送工厂函数数组，稍后会遍历执行其中的函数
		var list = structure[dataType],
			i = 0,
			length = list ? list.length : 0,
			// 当类型为请求发送器时，该类型为false，下面的for循环只会执行一次
		...
		for (; i < length && (executeOnly || !selection); i++) {
			...
			// 函数返回值的类型是字符串，即当前请求被重定向到了另一个数据类型
			if (typeof selection === "string") {
				// inspected[selection]为true表示对应的数据类型已经处理过
				// 递归调用的时候会传入，用来记录执行过的数据类型
	...
	jQuery.extend({
		...
		ajax: function (url, options) {
			...
			// 传入参数中有context则设置回调函数上下文
				...
				transport,
				// timeout handle
				// 请求如果超过指定的时间将会调用jqXHR.abort来取消本次请求
				timeoutTimer,
				// Cross-domain detection vars
				// 存放解析当前请求的地址得到的协议、域名或IP、端口，用于判断当前请求是否跨域
				parts,
				// The jqXHR state
				state = 0,
			...
			// 为参数url或选项url加上一个空字符串，强制将其转换为字符串，以支持值为document.location的情况
			// 如果以"//"开头，说明缺少协议部分，则添加location.href的协议部分
			// ajaxLocParts中存放了当前页面URL的协议、域名或IP、端口
	...
	// 将响应的数据转换为期望的类型
	// 从数据转换器集jQuery.ajaxSetting.converts中查找类型转换表达式对应的数据转换器，并执行
	// 当无法直接转换时会尝试查找可过度的数据类型
	// response 对于XMLHTTPRequest请求，可能是属性responseText或responseXML的值
	// 对于script请求，则是undefined
	...
	// 添加普通请求对应的请求发送器工厂函数，使用XMLHTTPRequest对象发送请求
	...
	jQuery.fn.extend({
		...
		// 负责执行一组样式动画，是所有动画效果的入口，其他便捷方法都是通过调用该方法来实现的
		animate: function (prop, speed, easing, callback) {
			...
			// 1. 遍历动画样式集合，修正、解析、备份样式
			// 2. 再次遍历动画样式集合，为每个样式构造标准动画对象，并开始执行动画
			...
			// 直接将doAnimation函数插入动画，利用了闭包特性
			// 也有可能直接执行动画函数，如果没有队列为false时
	...
	jQuery.fx.prototype = {
		...
		show: function () {
			...
			if (dataShow !== undefined) {
				...
				// 对于width和height，要修正开始值为1，以避免屏幕闪烁
